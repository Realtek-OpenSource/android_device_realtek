...
Client::Client(const sp<Sideband>& sideband)
    : mSideband(sideband),mGpuBWControl(NULL), mBWControlByHWC(true)
{
    char value[PROPERTY_VALUE_MAX];
    if(property_get("persist.rtk.hwc.bwcontrol", value, "1") && (!strcmp("0", (char *)value) || !strcmp("false", (char *)value)))
        mBWControlByHWC = false;
    mGpuBWControl = BWControl_create(BW_GPU);
}
...
status_t Client::requestHwSyncByFd(/*const sp<IBinder>& handle,*/ int32_t* resource, int audioHwSync) {
    ALOGV("[%s]",__FUNCTION__);
    return mSideband->requestHwSync(this, resource, audioHwSync);
}
...
status_t Client::getSidebandInfo(const sp<IBinder>& handle, String8 &info) {
    sp<SidebandLayer> layer = getLayerUser(handle);
    if (layer == NULL)
        return NAME_NOT_FOUND;

    layer->dump(info, " |              ", false);
    return NO_ERROR;
}

status_t Client::setHackSideband(const sp<IBinder>& handle, bool use) {
    sp<SidebandLayer> layer = getLayerUser(handle);
    if (layer == NULL)
        return NAME_NOT_FOUND;
    layer->setHackSideband(use);
    return NO_ERROR;
}
status_t Client::controlBW(void) {

    if(mBWControlByHWC) {
        if (mGpuBWControl) {
            ALOGI("[%s:%d]", __FUNCTION__, __LINE__);
            mGpuBWControl->LowPriority();
        }
    }
    return NO_ERROR;
}
