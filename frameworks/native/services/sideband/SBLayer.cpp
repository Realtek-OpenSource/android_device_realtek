...
SidebandLayer::SidebandLayer(Sideband* sideband, const sp<Client>& client,
                const String8& name, uint32_t flags, bool hack_sideband)
    :   mSideband(sideband),
        mName(name),
...
void SidebandLayer::onFirstRef() {
    ...
    if (property_get("vendor.feature-tunneled-playback", value, NULL)
...
void SidebandLayer::dump(String8& result, const char* prefix, bool dumpItemConsumer) const {
    ...
    if (mConsumer != NULL) {
        mConsumer->dump(result, mPrefix.string(), dumpItemConsumer);
    }
#if 0
...
void SidebandLayer::setHackSideband(bool use) {
    mHackSideband = use;
#ifndef PLATFORM_1GB
    if (mConsumer != NULL) {
        ALOGI("HackSideband setBufferQueue MaxAcquireBuffer");
