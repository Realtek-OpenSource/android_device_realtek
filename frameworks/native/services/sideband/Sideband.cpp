...
void Sideband::setHackSideband(bool hack_sideband) {
   mHackSideband = hack_sideband;
}
...
status_t Sideband::createLayer(const String8& name, const sp<Client>& client,
    ...
    layer->setKey((uint64_t) layer.get() ^ (uint64_t) systemTime(CLOCK_REALTIME));
...
status_t Sideband::requestHwSync(const sp<Client>& client, int32_t* resource, int audioHwSync) {
    ALOGI("[%s %d] client %p\n", __FUNCTION__, __LINE__, client.get());
    status_t err = NO_ERROR;
    if (mSBRefClock == NULL) {
        mSBRefClock = new SBRefClock();
    }
    err = mSBRefClock->requestHwSync(client, resource, audioHwSync);
    return err;
}
