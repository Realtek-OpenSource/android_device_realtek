...
status_t SBRefClock::importHwSyc(int32_t *resource, int fd)
{
    if (mRefClockfd >= 0)
        ion_close(mRefClockfd);
    mRefClockfd = fd;
...
status_t SBRefClock::requestHwSync(const sp<Client>& client, int32_t* resource, int audioHwSync)
{
    ALOGI("[%s %d] audioHwSync = %d", __FUNCTION__, __LINE__, audioHwSync);
    mRequestTime = systemTime(CLOCK_REALTIME);
    ALOGI("client %p mRequestTime %" PRId64 "\n", client.get(), mRequestTime);
    status_t err;
    if (audioHwSync >= 0)
        err = importHwSyc(resource, audioHwSync);
    else
        err = allocHwSyc(resource);
...
status_t SBRefClock::getHwSyncByResource(const sp<Client>& /*client*/, int32_t resource, int32_t* audioHwSync)
{
    if (resource == mRefClockfd) {
        ...
        ALOGI("nowNs %" PRId64 " mRequestTime %" PRId64 " diff %" PRId64, nowNs, mRequestTime, nowNs - mRequestTime);
