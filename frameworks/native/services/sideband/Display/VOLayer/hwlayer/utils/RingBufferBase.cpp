...
typedef struct
{
    ...
    int packet[256];//may need to modify later
...
void RingBufferBase::endianSwapMemoryCopy(BYTE* src, BYTE* des, unsigned long len)
{
    unsigned int i;
...
unsigned int RingBufferBase::recvInbandCmd(void *data, unsigned int size)
{
    ...
    if(nbyte >= (OSAL_S32)size){
        endianSwapMemoryCopy(rptr, (BYTE *)data, size);
        rptr += size;
        if(rptr >= upper)
            rptr = lower;
    }else{
...
{
    HRESULT* h;
    VIDEO_RPC_CHANNEL_LOW_DELAY info;
    ...
    h = VIDEO_RPC_ToAgent_ConfigChannelLowDelay_0(&info, &mClnt);
