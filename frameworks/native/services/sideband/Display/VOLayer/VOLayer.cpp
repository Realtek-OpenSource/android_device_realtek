...
//                        mLayerType = (get_hwc_linear_plane() == HWC_PROPERTY_PLANE_V1) ? LAYER_V2 : LAYER_V1;
    ...
    mGBInfoClient->setBufferHandle(handle);
    GraphicBufferInfo::private_yuv_data * pYUVData = mGBInfoClient->getYUVData();
    if (pYUVData->width >= 3840 || pYUVData->height >= 2160) {
        if (mGpuBWControl) {
...
status_t VOLayer::setFlags(uint32_t flags, uint32_t mask) {
    ...
    enum layer_type tmpLayer = LAYER_NULL;

    if(flags & SIDEBAND_V1){
        tmpLayer = LAYER_V1;
    }else if(flags & SIDEBAND_V2) {
        tmpLayer = LAYER_V2;
    }else if(flags & SIDEBAND_V3) {
        tmpLayer = LAYER_V3;
    }

    if(tmpLayer != LAYER_NULL && mLayerType != tmpLayer){
        mFlags |= FIRST_RENDER;
//        mWindow->setFlags(HIDDEN|REFRESH,HIDDEN|REFRESH);
        mHWLayer->disconnect();
        mLayerType = tmpLayer;
