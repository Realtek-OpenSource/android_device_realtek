...
#if defined(ENABLE_TEE_DRM_FLOW) && defined(USE_DVB_SVP_FLOW)
...
typedef struct
{
    ...
    unsigned int   colorMap_offset[9];
    DVBSP_TARGET   target[DVBSP_MAX_NUM_TARGETS] ;	
    unsigned int   regionBuf_phy ;

	VIDEO_VO_OBJ_DVB_SP_REGION_SVP objSVP;
...
typedef struct
{
    ...
    SP_INFO *pSpInfo;
    unsigned char dump_cnt;
	bool is_svp;
#if defined(ENABLE_TEE_DRM_FLOW) && defined(USE_DVB_SVP_FLOW)
    ION_DATA m_ion_svp;
    ION_DATA m_data_svp;
	DVBSP_DECODE_SVP *Dec_SVP;
	DVBSP_DECODE_SVP *Dec_SVP1;
	
    TEEC_Session *DVB_tee_session;
    RTK_TEEC_Session *DVB_tee_rtk_sess;
#endif
} DVBSP_DECODE ;
...
void DVBSP_SwapPacketToVO(VIDEO_VO_OBJ_DVB_SP_REGION* regionObj, int regionIdx, int last_in_page, bool is_svp);
#if defined(ENABLE_TEE_DRM_FLOW) && defined(USE_DVB_SVP_FLOW)
TEEC_Session *rvsd_get_tee_see(void *pRvsd);
RTK_TEEC_Session *rvsd_get_rtk_tee_see(void *pRvsd);
bool rvsd_get_is_svp(void *pRvsd);
#endif
#endif
