...
#if (BDSP_ON_MIPS == 0)
    /* Phoenix */
    unsigned int   bsCurr_C ;   // unit: bits
    SP_INFO *pSpInfo;
#endif
...
long BDSP_ToMipsUncacheAddrFunc(ION_DATA *pIon, unsigned char *pVirt);
int BDSP_dbg_dump_pic_Index(VP_COMPONENT *_this);
int BDSP_dbg_dump_pic_argb8888(VP_COMPONENT *_this);
int BDSP_dbg_dump_decoded_pixels(BDSP_DECODE *Dec, char *pixel_addr, int width
    , int height, int pitch);
int BDSP_malloc_dec_memory(VP_COMPONENT *_this);
int BDSP_free_dec_memory(VP_COMPONENT *_this);
int BDSP_Draw_Pixel (BDSP_DECODE *Dec, BDSP_TARGET *target, unsigned int pixel /* always 8 bits */, unsigned int len);
