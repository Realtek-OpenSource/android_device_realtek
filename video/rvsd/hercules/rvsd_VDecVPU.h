...
#ifdef RVSD_VP9_NEW_FLOW
#define MAX_BS_LINE_BUF 5
#endif

#if defined(RVSD_HANDLE_ERRBLK_AV_SYNC)
enum HANDLE_ERRBLK_STEP
{
    HANDLE_ERRBLK_STEP_NONE,
    HANDLE_ERRBLK_STEP_CHECK_FREERUN,
    HANDLE_ERRBLK_STEP_CHECK_AV_SYNC,
    HANDLE_ERRBLK_STEP_COMPLETED
};
#endif
...
typedef struct
{
    ...
    unsigned char       *pVirBase;  // virtual address of ring buffer start
    unsigned char       *pVirLimit; // virtual address of ring buffer end
    unsigned long       phyBase;    // physical address of ring buffer start
    unsigned long       phyLimit;   // physical address of ring buffer end
    int                 size;       // the size of ring buffer
} VPU_BS_BUF_INFO;
...
struct _DecConfigParam
{
    ...
    unsigned int newSegWPtrDV;  // for Dolby Vision, save the reset ring buffer rp when start play or after flush to newSegWPtrDV to be newSegWPtr
    ...
    unsigned short bIsVideoSlowForErrBlk;
    ...
    long long flushLogCtrlPTS;
    long long blWaitingCtrlPTS;
    long long splitterEosLogCtrlPTS;
    long long qFullLogCtrlPTS;
...
#if defined(DUMP_DOLBY_METADATA) && defined(ENABLE_DOLBY_METADATA_PARSER)
    unsigned int metadata_file_count;
    bool is_open_metadata;
#endif
    ...
    //bool            bContinueStartOneFrameForCQ;        // set false when flush done, set true after 1st feed data after NewSeg.
...
#ifdef RVSD_VP9_NEW_FLOW
    /* vp9 start */
    vpu_buffer_t    vbStreamPicEnd[MAX_BS_LINE_BUF];
    void        *pStreamBuf;
    void        *pStreamBufReuse;
    ...
    struct IONStruct pIonBsPicEnd[MAX_BS_LINE_BUF];
    /* vp9 end */
#endif
    bool            bSkipDue2ErrBlock; // bSkipDue2ErrBlock is for RTCEZ-143
#if defined(RVSD_HANDLE_ERRBLK_AV_SYNC)
    HANDLE_ERRBLK_STEP  handleErrBlkStep;
    bool                bPrepareErrBlkAVSync;
    unsigned long       orgVideoFreeRunThreshold;
    long long           lastSendFrmPTSBfErrBlk;
    long long           newSendIFrmPTSAfErrBlk;
#endif
    bool            bCtrPathValid;
    int             nCQUsingCnt;
} ;
