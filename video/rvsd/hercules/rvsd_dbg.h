...
// 1. If not Dolby Vision, dump data from video ring buffer (between DvdPlayer and RVSD) before feeding data to VPU ring buffer.
// 2. If Dolby Vision, dump data from Splitter BL/EL ring buffer before feeding data to VPU ring buffer.
#define _DBG_DUMP_BS_EN 0
#if _DBG_DUMP_BS_EN
#ifdef ENABLE_TEE_DRM_FLOW
// For Debug Only!
// MUST enable DBG_SECURE_BS_DUMP of TEE API(Tee_api.c,TEE_API_BitstreamPrint/TEE_API_BitstreamOut)
// to support video bitstream print or dump for secure bitstream buffer type
#define _DBG_DUMP_SECURE_BS_EN 1
//IF choose print instead of file dump, just setting -1 or print count number 
#define _DBG_SECURE_BS_PRINT_CNT 0
#endif
#endif
// Only Dolby Vision, dump BL/EL/MD data in nal_dmx_cb_func() from video ring buffer (between DvdPlayer and RVSD) before feeding data to Splitter BL/EL ring buffer.
#define _DBG_DUMP_SPLITTER_BL_EL_MD 0
// Only Dolby Vision, dump VES data in Splitter::VesDmx() from video ring buffer (between DvdPlayer and RVSD) before feeding data to Dolby Vision ves splitter library.
#define _DBG_DUMP_SPLITTER_VES      0
...
#define ONLY_RESERVE_NEWEST_DUMP_BS_BYTES 0//(50*1024*1024) // Only reserve the newest dump files and the file size will not exceed this bytes.
...
#define _DBG_CHECK_IPC_SPEND_TIME 0
...
#define _DBG_DUMP_SECURE_BS_NEW(a, b, c, d)   _dbg_dump_secure_bs_new(a, b, c, d)
#else
#define _DBG_DUMP_SECURE_BS_NEW(a, b, c, d)
#endif
...
// function prototype //
////////////////////////
#if defined(DUMP_DOLBY_METADATA)
class DolbyMetadataItem;
#endif
...
#if defined(DUMP_DOLBY_METADATA)
void DumpDolbyMetadata(const void *p_vpu, const DolbyMetadataItem &data, const char *p_source_path = NULL);
#endif
