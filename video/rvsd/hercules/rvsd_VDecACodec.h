...
#if defined(RVSD_ACODEC_FLOW_SIDEBAND_ENABLED) || defined(RVSD_ACODEC_FLOW_TUNNELED_PLAYBACK_ENABLED)
#include <SidebandClient.h>
#include <SidebandControl.h>
#include <utils/NativeHandle.h>
#endif
#ifdef RVSD_ACODEC_FLOW_HACKED_SIDEBAND_ENABLED
...
//#define RVSD_ACODEC_DEBUG_TRACE_FRAME
//#define RVSD_ACODEC_DEBUG_TRACE_SYNC
#ifdef RVSD_ACODEC_DEBUG_TRACE_FRAME
#define RVSD_ACODEC_DBG_TRACE_FRAME(fmt, args...) ALOGD(fmt, ##args)
#else
#define RVSD_ACODEC_DBG_TRACE_FRAME(fmt, args...)
#endif
#ifdef RVSD_ACODEC_DEBUG_TRACE_SYNC
#define RVSD_ACODEC_DBG_TRACE_SYNC(fmt, args...) ALOGD(fmt, ##args)
#else
#define RVSD_ACODEC_DBG_TRACE_SYNC(fmt, args...)
#endif
...
#ifdef ANDROID_6
    VDecController(RVSD_INFO* pRvsd_info, const sp<Surface> &surface, const sp<MediaSource> &source, sp<AHandler> VOid, int32_t SB_audio_sessionid);
#else
    VDecController(RVSD_INFO* pRvsd_info, const sp<Surface> &surface, const sp<MediaSource> &source, ALooper::handler_id VOid, int32_t SB_audio_sessionid);
#endif
    void startAsync();
    void stopAsync();
    void signalFlush();
...
private:
...
#endif
    int32_t mSB_Audio_Sessionid;// audio session ID for tunneled playback from SB serveice, FOR REFERENCE CLOCK AV SYNC USED
    ...
    int64_t mStartTimeUs_fps;
    int32_t mNumOutputBuffersReceived;
    int32_t mNumOutputBuffersReceived_fps;
...
private:
    ...
    //void disconnectNativeWindow();
#ifdef RVSD_ACODEC_FLOW_HACKED_SIDEBAND_ENABLED
    void save_prop_hacked_sideband();
    void restore_prop_hacked_sideband();
#endif
    enum {
        kWhatVideoNotify                = 'vidr',
    };
...
#endif
#if defined(RVSD_ACODEC_FLOW_HACKED_SIDEBAND_ENABLED)
    char hacked_sideband_prop[PROPERTY_VALUE_MAX];
    char hacked_sideband_compr_mode_prop[PROPERTY_VALUE_MAX];
#endif
};
