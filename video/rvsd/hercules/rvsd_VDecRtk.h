...
#define SUPPORT_WATCHDOG_ERROR_HANDLE
...
#define RVSD_HANDLE_ERRBLK_AV_SYNC
...
//#define CHECK_BROKEN_LINK // default disable due to RVSD always search I frame first, so doesn't need check broken link
...
#define RVSD_DOLBY_VISION_PAIR_FRM_BY_DERIVED_PTS_ENABLE
#define RVSD_FEED_SIZE_FIXED_EN 0
#define RVSD_FEED_SIZE_FIXED    (128*1024)
...
#define DV_LAYER_STR(pSplitter, isBLFlag)   (((pSplitter)==NULL)?(""):(((isBLFlag)==true)?("BL."):("EL.")))
...
int LoadVDecFw(int core_index, bool is_svp, void* sess=NULL, void* rtk_sess=NULL);
...
#endif // #ifndef RVSD_NEW_VIDEO_FLOW_EN

struct ElDecodeContext
{
    ElDecodeContext()
    {
        thread_id = -1;
    ...
    pthread_t thread_id;
    RVSD_INFO *p_rvsd_info;
    void *p_feeder;         // point to *mp_feeder of base decoder
    void *p_icq_ion;       // point to RvsdIonAllocator object
};


struct ComposerRegister
{
};

struct DmCore1Register
{
};

struct DmCore2Register
{
};

struct DmCore3Register
{
};


struct DolbyVoRegister
{
    ComposerRegister composer;
    DmCore1Register core1;
    DmCore2Register core2;
    DmCore3Register core3;
};

class VDecRtk : public RefBase {
public:
    VDecRtk(RVSD_INFO* pRvsd_info, pthread_t *pThreadVideo, ElDecodeContext *p_el_context = NULL);
    int VDecInit(void);
    void VDecUninit(void);
    void UnInit(void);
    void *VDec_decode_loop(void);
    void *VDec_showVO_loop(void);
    void *DolbyParserTask();
    ...
    void* GetFrameQueueInfo(int indexFrameDisplay);
    virtual ~VDecRtk();
#if defined(RVSD_HANDLE_ERRBLK_AV_SYNC)
    void *VDec_errblk_avsync(void);
#endif
...
private:
    ...
    void FillYUVData(YUV_STATE* yuv_win, void* pdisplayInfo, void* pdisplayInfoR, long long outTimestamp, long long RPTS, long long outTimestamp2, long long RPTS2, unsigned long context, int mode, AspectRatioStruct *ar, char IsReSendFrm, void *p_el_display_info, const void *p_metadata);
    void SendOneFrameToVO(YUV_STATE* yuv_win, void* pdisplayInfo, void* pdisplayInfoR, long long outTimestamp, long long RPTS, unsigned long context, char IsReSendFrm, void *p_el_display_info, const void *p_dolby_register);
    ...
    void IsPlockSetEmpty(bool &current, bool &previous);
    void FreeCurrentFb();
...
#if !defined(FIX_DHCHERC_667)
    void UpdateSeqInitConsumedBytes(PhysicalAddress seqInitRdPtr);
#endif
    int IsUserDataExist(void *pDecInfo, int *pUserDataSize);

    bool IsHandlingVES();

    void CreateElDecoder(void *p_feeder);
    void DestroyElDecoder();

    void SetDolbyVisionDecodeOption(void *p_vpu, const ENUM_MEDIA_TYPE codec);

    void SetElDecStatus(ENUM_VIDEO_DEC_STATUS status);
    ...
    bool GetElDisplayInfo(void *p_current_display_info,
                          void *p_next_display_info,
                          long long *p_rpts,
                          unsigned long *p_context,
                          char *p_is_resend_frame);
    void PopDispQueue();
    void PeekDispQueue(void *info);    // DecOutputInfo *info

    void CreateDolbyMetadataParser();
    void DestroyDolbyMetadataParser();

    void* GenerateDolbyRegisters(const void *p_metadata, const void *p_bl_displayinfo, const void *p_el_display_info);
    void GetHdrFromInfoframe(YUV_STATE *p_yuv);

    bool IsFel(const void *p_metadata) const;
    void DisableDolbyFlowForFel(RINGBUFFER_HEADER old_BSHDRBase);

#if defined(RVSD_ENABLE_VES_SPLITTER)
    void SetVideoInfoToSplitter();
#endif
    ...
    VDecRtk *mp_el_decoder;
    ElDecodeContext m_el_context;
...
#endif
#if _DBG_DUMP_SECURE_BS_EN
    void _dbg_dump_secure_bs_new(void *sess, PhysicalAddress Ptr1, PhysicalAddress Ptr2, int print_cnt);
#endif
